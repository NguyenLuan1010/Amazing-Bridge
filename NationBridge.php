...
//Hiển thị bài viết theo quốc gia
if(isset($_POST['national'])){
    $nationID=$_POST['national'];
    $query14="SELECT b.*,a.National_Name FROM quốc_gia a JOIN bridges_posts b  where 
   a.NationalID=b.NationalID and b.NationalID='$nationID'";
...
<head>
    ...
    <?php foreach($result14 as $item):?>
      <title>Quốc Gia-<?=$item['National_Name']?></title>
    <?php endforeach;?>
...
<?php foreach($result14 as $item):?>
<body class="nationBackground" style="  background-image: url('Images/Big_images/<?=$item['BBackImages']?>');">
<?php endforeach;?>
<div class="header-box">
    ...
    <div class="nationBody">
    <?php if($nationID != 'Quốc Gia' ):?>
        <?php foreach($result14 as $item):?>
            <a href="ConceptBridges.php?BPostID=<?=$item['BPostID']?>" class="NationBox">
                <img src="Images/Normal_images/<?=$item['BNormalImages']?>" alt="">
                <div class="searchContent">
                    <br> <h2 ><?=$item['BTitle']?></h2><br>
                    <p><?=$item['BHeadPost']?></p>
                </div>
           </a>
        <?php endforeach;?>
     <?php else:?>  
          <div class="noneNation">
            <h2>Hãy chọn một quốc gia </h2><br>
            <form action="NationBridge.php" method="post">
                    <select  name="national" >
                        <option hidden > Quốc Gia</option>
                         <?php foreach($result1 as $item):?>
